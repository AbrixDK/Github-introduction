...
# Trigger the Romania/Iraq workflow after it was added to the base branch.
...
OUT=ROOT/'romania_iraq_output'; VIDEO=OUT/'video'; SHEETS=OUT/'sheets'; TMP=OUT/'tmp'
...
FILES=[
 ('romania_2017-06-09','President Trump Meets with the President of Romania.webm',70),
 ('iraq_2017-03-20','President Trump Meets With Iraqi Prime Minister at the White House.webm',50),
]
...
def main():
    rows=[]
    for key,title,maxlen in FILES:
        rec={'key':key,'title':title,'page_url':'https://commons.wikimedia.org/wiki/File:'+title.replace(' ','_')}
        try:
            ii=info(title); url=choose(ii); rec['download_url']=url; rec['original_url']=ii.get('url')
            ...
            pat=TMP/f'{key}_%05d.jpg'; vf=("fps=10,scale=800:-2,drawtext=fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf:text='%{pts\\:hms}':x=10:y=10:fontsize=28:fontcolor=white:box=1:boxcolor=black@0.65")
            run(['ffmpeg','-loglevel','error','-y','-i',str(path),'-t',str(min(maxlen,dur)),'-vf',vf,'-q:v','3',str(pat)])
