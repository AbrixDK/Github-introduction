...
# Trigger the Swedish workflow after it was added to the base branch.
import json, subprocess, time
from pathlib import Path
import requests

API='https://commons.wikimedia.org/w/api.php'
TITLE='President Trump Meets with Prime Minister Stefan Löfven.webm'
ROOT=Path(__file__).resolve().parent
OUT=ROOT/'lofven_output'; VIDEO=OUT/'video'; SHEETS=OUT/'sheets'; TMP=OUT/'tmp'
...
def main():
    ii=get_info(); url=choose(ii); ext=Path(url.split('?',1)[0]).suffix or '.webm'; path=VIDEO/('lofven'+ext); download(url,path)
    ...
    pat=TMP/'fine_%05d.jpg'; vf=("fps=10,scale=800:-2,drawtext=fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf:text='%{pts\\:hms}':x=10:y=10:fontsize=28:fontcolor=white:box=1:boxcolor=black@0.65")
    run(['ffmpeg','-loglevel','error','-y','-i',str(path),'-vf',vf,'-q:v','3',str(pat)])
    run(['ffmpeg','-loglevel','error','-y','-framerate','10','-i',str(pat),'-vf','scale=320:-2,tile=5x5:padding=4:margin=4','-q:v','3',str(SHEETS/'lofven_fine_%03d.jpg')])
