...
TITLE='President Donald Trump Meets with Prime Minister Erna Solberg.webm'
ROOT=Path(__file__).resolve().parent
OUT=ROOT/'solberg_output'; VIDEO=OUT/'video'; SHEETS=OUT/'sheets'; TMP=OUT/'tmp'
...
def main():
    ii=get_info(); url=choose(ii); ext=Path(url.split('?',1)[0]).suffix or '.webm'; path=VIDEO/('solberg'+ext); download(url,path)
    ...
    run(['ffmpeg','-loglevel','error','-y','-i',str(path),'-t',str(min(40,dur)),'-vf',vf,'-q:v','3',str(pat)])
    run(['ffmpeg','-loglevel','error','-y','-framerate','10','-i',str(pat),'-vf','scale=320:-2,tile=5x5:padding=4:margin=4','-q:v','3',str(SHEETS/'solberg_fine_%03d.jpg')])
